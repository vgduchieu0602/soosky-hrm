...
import RefreshTokenDocument from "@modules/auth/adapters/driven/persistence/mongodb/documents/RefreshTokenDocument";
...
const DEFAULT_TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 ngày

/**
 * Hiện thực `RefreshTokenStore` trên MongoDB.
 *
 * Token thô là 32 byte ngẫu nhiên (base64url), chỉ trả cho client; DB chỉ lưu
 * SHA-256 của token làm `_id` — token đủ entropy nên không cần hàm băm chậm
 * như mật khẩu, và tra cứu là so khớp chính xác trên khoá chính.
 *
 ...
 */
export default class MongoRefreshTokenStore extends MongoRepository<RefreshTokenDocument> implements RefreshTokenStore {
    ...
    public constructor(
        ...
        super(db, AUTH_COLLECTIONS.refreshTokens, session);
    }
    ...
    public static async ensureIndexes(db: Db): Promise<void> {
        const collection = db.collection<RefreshTokenDocument>(AUTH_COLLECTIONS.refreshTokens);
    ...
    public async revoke(token: string): Promise<void> {
        await this._collection.deleteOne({ _id: this._hash(token) }, this._sessionOptions);
    }
